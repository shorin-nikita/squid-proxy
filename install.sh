...
SCRIPT_URL="https://raw.githubusercontent.com/shorin-nikita/squid-proxy/main/install.sh"

# Если stdin не терминал (запуск через curl | bash), скачиваем себя и перезапускаем
if [ ! -t 0 ]; then
    TEMP_SCRIPT=$(mktemp)
